...
<form class="forms" action="./?action=register" method="POST">
    ...
    value="<?php if(!empty($_POST['name'])) echo $_POST['name']; ?>" required maxlength="20" /><br />

    <label for="firstname">Mon prénom</label>
    <input type="text" name="firstname" placeholder="Prénom" 
    value="<?php if(!empty($_POST['firstname'])) echo $_POST['firstname']; ?>" required maxlength="20" /><br />

    <label for="mail">J'entre mon identifiant</label>
    <input type="email" name="mail" placeholder="Mon Email" 
    value="<?php if(!empty($_POST['mail'])) echo $_POST['mail']; ?>" required maxlength="50"/><br />

    <label for="password">J'entre mon mot de passe</label>
    <input type="password" name="password" placeholder="min. 8 caractères, dont 1 caractère spécial" required minlength="8" pattern="(?=.*[!@#$%^&*()\-_=+{};:,<.>])\S+" /><br />
